...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run a single training and evaluation run"
    )
    ...
    parser.add_argument(
        "--layers", type=int, default=2, help="Number of layers in the model"
    )
...
if __name__ == "__main__":
    ...
    train_set = dset_factory(args.dataset_path, "train")
    val_set = dset_factory(args.dataset_path, "val")
    test_set = dset_factory(args.dataset_path, "test")

    print(
        f"Dataset samples: Train: {len(train_set)}, Val: {len(val_set)}, Test: {len(test_set)}"
    )
    print("First train sample:", train_set[0])
    print("Label map:", train_set.label_map)
    ...
    with mlflow.start_run(run_name=args.mlflow_run):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if early_stopping(
                ...
                print(
                    f"Early stopping at epoch {epoch} with min val_loss: {early_stopping.best_loss:.4f}"
                )
                break
